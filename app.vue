<template>
	<main>
		...
		<section class="list">
			...
			>
				<PersonalCard v-bind:person="person" @removeWasClicked="removePerson" @editWasClicked="editPerson"></PersonalCard>
			</section>
		</section>
		<div v-show="state != false" class="inputs">
			<input v-model="name" placeholder="First Name" />
			<input v-model="surname" placeholder="Last Name" />
			<input v-model="birthyear" placeholder="Year of birth" />
		</div>
		<div class="buttonHolder">
			...
			<div v-show="state != false" class="button" id="clear-input-button" @click="clearInput">Clear Imput</div>
		</div>
	</main>
</template>
...
<script setup>
	...
	const removePerson = (personShouldBeRemoved) => {
		persons.value = persons.value.filter((item) => {
			console.log(personShouldBeRemoved)
			console.log(item)
